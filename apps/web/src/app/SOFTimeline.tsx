...
function AnnotationFlag({ evLabel, desc, color, bg }: AnnotationFlagProps) {
  return (
    ...
    >
      ...
      <p style={{ fontSize: "11px", color: "#374151", lineHeight: 1.45 }}>
        {desc}
      </p>
    </div>
  );
...
function readManualEventFormSnapshot(
  ...
  const readGlobal = (selector: string) =>
    (
      form.ownerDocument?.querySelector<
        HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement
      >(selector)?.value ?? ""
    ).trim();
  ...
  return {
    ...
    deductible: Boolean(
      form.querySelector<HTMLInputElement>("#sof-event-deductible")?.checked,
    ),
...
function createLocationEvidenceForm(
  ...
  return {
    evidenceTime: formatDateTimeInputInTimeZone(
...
const initialEvents: TimelineRow[] = [
  ...
  },
  {
    n: "02",
    state: "normal",
    timestamp: "23 Oct 14:30",
    name: "Laytime commences",
  ...
  },
  {
    n: "03",
    state: "normal",
    timestamp: "24 Oct 02:15",
    name: "Berthing completed",
  ...
  },
  {
    n: "05",
    state: "deductible",
    timestamp: "24 Oct 11:20",
    name: "Rain squall — operations suspended",
    detail: "Operations halted due to adverse weather conditions",
    tag: "Exception candidate",
    tagBg: "#F3F4F6",
    tagText: "#374151",
    duration: "2h 30m",
    cause: "Weather",
  ...
  },
  {
    n: "06",
    state: "normal",
    timestamp: "24 Oct 13:50",
    name: "Loading resumed",
  ...
  },
  {
    n: "07",
    state: "deductible",
    timestamp: "25 Oct 03:40",
    name: "Terminal breakdown — arm fault",
  ...
  },
  {
    n: "10",
    state: "pending",
    timestamp: "26 Oct 00:00",
    name: "Completion to NOR — under review",
...
function intervalStringToSeconds(value?: string | null) {
  ...
  const match = String(value)
    .trim()
    .match(/^(?:(-?\d+)\s+days?\s+)?(\d{1,2}):(\d{2}):(\d{2})$/);
...
function getCalculationSnapshot(
  ...
  return (
    (calc?.decisionSnapshot as LaytimeDecisionSnapshot | null | undefined) ??
    null
  );
}
...
function getEvidenceAuditIds(calc?: LaytimeCalculation | null) {
  ...
  if (commencement?.readinessEventId)
...
function validateManualEventForm(
  ...
  if (
    form.sourceTimeZone.trim() &&
    !isValidIanaTimeZone(form.sourceTimeZone.trim())
  )
    return "Enter a valid IANA source timezone.";
  ...
  if (
    form.operation &&
    form.operation !== "Loading" &&
    form.operation !== "Discharge"
  ) {
...
function getInputSnapshot(calc?: LaytimeCalculation | null) {
  return (
    ((calc as any)?.inputSnapshot as
      LaytimeCalculation["inputSnapshot"] | undefined | null) ?? null
  );
}

function getOperationResultSnapshot(result?: LaytimeOperationResult | null) {
  return (
    ((result as any)?.inputSnapshot?.operationResult as
      | {
          source?: string | null;
...
function getSofDocumentSelectionAudit(calc?: LaytimeCalculation | null) {
  ...
  const candidateDocumentIds = Array.isArray(
    documentSelection?.candidateDocumentIds,
  )
    ? documentSelection.candidateDocumentIds
    : [];
  const includedDocumentIds = Array.isArray(
    documentSelection?.includedDocumentIds,
  )
    ? documentSelection.includedDocumentIds
    : [];
  const matchingDocumentIds = Array.isArray(
    documentSelection?.matchingDocumentIds,
  )
    ? documentSelection.matchingDocumentIds
    : [];
  const legacyNullDocumentIds = Array.isArray(
...
function getWeatherWorkingAudit(calc?: LaytimeCalculation | null) {
  ...
  return {
    ...
    totalWeatherTimeDeductedBeforeDemurrage:
      typeof weatherWorking?.totalWeatherTimeDeductedBeforeDemurrage ===
      "number"
...
function getReversibleLaytimeAudit(calc?: LaytimeCalculation | null) {
  ...
  const ruleEnabled =
    typeof reversibleRule?.enabled === "boolean"
      ? reversibleRule.enabled
      : null;
  ...
  return {
    available: Boolean(
      snapshot && (reversibleRule || reversibleAnalysis || settlement),
    ),
    ...
    warnings: [
      ...
      ...(Array.isArray(reversibleRule?.warnings)
        ? reversibleRule.warnings
        : []),
    ],
  };
}
...
const ENGINE_EVENT_PRESETS = [
  ...
  {
    value: "VESSEL_READY_IN_ALL_RESPECTS",
    label: "Vessel ready in all respects",
  },
...
function inferState(
  eventType: string,
  parsed: ManualEventDetails | null,
  isManualOverride: boolean,
): EventState {
  ...
  return (
    <div>
      <div
        ...
        style={{ height: "10px", backgroundColor: "#F3F4F6" }}
      >
        {segments.map((s, i) => (
          <div
            key={i}
      ...
      <div className="flex justify-between mb-3">
        {["23 Oct 08:00", "24 Oct 00:00", "25 Oct 00:00", "26 Oct 00:00"].map(
          (d) => (
            <span key={d} style={{ fontSize: "9px", color: "#9CA3AF" }}>
              {d}
            </span>
          ),
        )}
...
function AddEventModal({
  ...
  const getInitialForm = (): ManualEventForm => {
    ...
    return {
      eventTime: formatDateTimeInputInTimeZone(
        event?.eventTimeIso ?? new Date().toISOString(),
      ...
      duration:
        event?.duration && event.duration !== "â€”" ? event.duration : "",
  ...
  const saveDisabledMessage =
    validationError ?? (!isDirty ? "Make a change before saving." : null);
      ...
      >
        ...
        >
          <div>
            ...
            <p style={{ fontSize: "11px", color: "#6B7280", marginTop: "3px" }}>
              Required for new authoritative timestamps. Legacy events without
        ...
        <form
          ...
          onSubmit={(e) => {
            ...
            const snapshot = formRef.current
              ? readManualEventFormSnapshot(formRef.current, mode)
              : form;
        ...
        >
              ...
              onChange={(e) =>
                setForm((current) => ({
                  ...
                  operation: e.target.value
                    ? (e.target.value as EventOperation)
                    : null,
                }))
              }
              className="mt-1 w-full rounded-md border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500"
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                value={form.cause}
                onChange={(e) =>
                  setForm((current) => ({ ...current, cause: e.target.value }))
                }
                className="mt-1 w-full rounded-md border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500"
                style={{ borderColor: "#D1D5DB" }}
              />
          ...
          <p style={{ fontSize: "11px", color: "#6B7280", lineHeight: 1.45 }}>
            Actual deductible time is determined by Charter Party rules during
            the laytime calculation.
          </p>

          <div>
            <label
              htmlFor="sof-event-notes"
              style={{ fontSize: "11px", color: "#374151", fontWeight: 500 }}
            >
              Notes
            </label>
...
export default function SOFTimeline() {
  ...
  const [laytimeCalculation, setLaytimeCalculation] =
    useState<LaytimeCalculation | null>(null);
  ...
  const [locationEvidenceError, setLocationEvidenceError] = useState<
    string | null
  >(null);
  const [showLocationEvidenceForm, setShowLocationEvidenceForm] =
    useState(false);
  ...
  useEffect(() => {
    ...
    async function loadTimeline() {
      ...
      try {
        const documentsResponse = await getSofDocuments(id, {
          page: 1,
          limit: 50,
        });
        if (!alive) return;

        const sortedDocuments = [...(documentsResponse.data ?? [])].sort(
          (a, b) => {
        ...
        const eventsResponse = await getSofEvents(activeDocument.id, {
          page: 1,
          limit: 200,
        });
        if (!alive) return;
  ...
  const displayEvents = (id ? timelineEvents : initialEvents).map((event) => ({
    ...event,
    evidenceStatus:
      event.eventId && evidenceAudit.selectedIds.has(event.eventId)
        ? ("selected" as const)
  ...
  async function handleSaveEvent(
    ...
    try {
      ...
      const payload = {
        ...
        ...(form.sourceTimeZone.trim()
          ? { sourceTimeZone: form.sourceTimeZone.trim() }
          : {}),
      ...
      if (editTarget) {
        ...
        const eventChanged =
          nextEventTime !== editTarget.eventTimeIso ||
          nextEventType !== (editTarget.eventType ?? "");
      ...
      setTimelineSuccess(
        editTarget
          ? "SOF event updated successfully."
          : "SOF event created successfully.",
      );
      setShowAddEvent(false);
      setEditingEvent(null);
      setRefreshKey((current) => current + 1);
    } catch (error: any) {
      const message = sofErrorMessage(
        error,
  ...
  async function handleSaveLocationEvidence() {
    ...
    setTimelineSuccess(null);
    try {
      if (!locationEvidenceForm.sourceTimeZone.trim()) {
        throw new Error("Enter the source timezone for this observation.");
      }
      await createNorTenderLocationEvidence(id, {
        evidenceTime: resolveLocalDateTimeInTimeZone(
          locationEvidenceForm.evidenceTime,
          locationEvidenceForm.sourceTimeZone.trim(),
        ),
        sourceTimeZone: locationEvidenceForm.sourceTimeZone.trim(),
  ...
  const sourceOperationText = sourceOperation
    ? `Operation: ${formatOperationLabel(sourceOperation)}`
    : null;
  ...
  const shipmentMeta = [
    shipment?.port,
    shipment?.cargoType,
    shipment?.cargoQuantity != null
      ? `${Number(shipment.cargoQuantity).toLocaleString()} MT`
      : null,
  ...
  const sofDocumentSelectionAudit =
    getSofDocumentSelectionAudit(laytimeCalculation);
  const demurrageAudit = getDemurrageAudit(laytimeCalculation);
  const operationSelectionAudit =
    getOperationSelectionAudit(laytimeCalculation);
  ...
  const netUsedSeconds =
    grossUsedSeconds !== null
      ? Math.max(0, grossUsedSeconds - deductionSeconds)
      : null;
  ...
  const demurrageAmount = formatCurrencyAmount(
    laytimeCalculation?.demurrageAmount,
    calculationCurrency,
  );
  const despatchAmount = formatCurrencyAmount(
    laytimeCalculation?.despatchAmount,
    calculationCurrency,
  );
  const nonReversibleSettlement =
    calculationSnapshot?.nonReversibleSettlement ?? null;
  ...
  const netPositionValue = nonReversibleSettlement
    ...
    : laytimeCalculation &&
        Number((laytimeCalculation as any)?.demurrageAmount) > 0
      ? `${demurrageAmount} demurrage`
      : laytimeCalculation &&
          Number((laytimeCalculation as any)?.despatchAmount) > 0
        ? `${despatchAmount} despatch`
        : laytimeCalculation
          ? formatCurrencyAmount(0, calculationCurrency)
          : "—";
  const supplierClockStart = formatDateTime(
    calculationSnapshot?.commencement?.commencedAt,
  );
  const cargoCompletionValue = formatDateTime(
    evidenceAudit.completion?.selectedTime ??
      evidenceAudit.completion?.eventTime,
  );
  ...
  const demurrageAmountValue = Number(
    (laytimeCalculation as any)?.demurrageAmount ?? 0,
  );
  const despatchAmountValue = Number(
    (laytimeCalculation as any)?.despatchAmount ?? 0,
  );
  const reversibleSettlement =
    calculationSnapshot?.reversibleSettlement ?? null;
  const reversibleConfigured =
    Boolean(reversibleSettlement) ||
    calculationSnapshot?.reversibleLaytimeRule?.enabled === true;
  const reversibleSettlementStatus = (reversibleSettlement?.settlementStatus ??
    (reversibleConfigured
      ? "LEGACY"
      : null)) as ReversibleSettlementStatus | null;
  ...
  async function handleCreateClaim() {
    ...
    try {
      const type =
        demurrageAmountValue > 0 ? "demurrage_counter" : "despatch_claim";
      const amountDisputed =
        demurrageAmountValue > 0 ? demurrageAmountValue : despatchAmountValue;
  ...
  const allowedDisplay = singleOperationSummary
    ...
    : nonReversibleMultiOperationSummary || nonAuthoritativeReversibleSummary
      ? nonAuthoritativeReversibleSummary
        ? "Not authoritative — see operation results"
  ...
  const deductionsDisplay = hasLaytimeCalculation
    ? formatSecondsAsInterval(deductionSeconds)
    : "—";
  const netUsedDisplay = hasLaytimeCalculation
    ? formatSecondsAsInterval(netUsedSeconds)
    : "—";
  const remainingDisplay = hasLaytimeCalculation
    ? formatSecondsAsInterval(remainingSeconds)
    : "—";
  const overrunDisplay = hasLaytimeCalculation
    ? formatSecondsAsInterval(overrunSeconds)
    : "—";
  ...
  const supplierClockNote = laytimeCalculation
    ...
    : laytimeLoading
      ...
      : "No persisted laytime calculation yet.";
  const defaultManualEventOperation =
    activeDocument?.operation ?? shipment?.laytimeOperation ?? "Discharge";
  const norTenderEvents = timelineEvents.filter(
    (event) => event.eventType === "NOR_TENDERED" && event.eventId,
  );
  const selectedNorTenderEvent = norTenderEvents.find(
    (event) => event.eventId === locationEvidenceForm.norTenderedEventId,
  );
  ...
  return (
    ...
    >
          ...
          <>
            ...
            <button
              ...
              style={{
                ...
                backgroundColor: "#1A4ED8",
                border: "none",
                opacity: laytimeRunning ? 0.7 : 1,
              }}
              disabled={laytimeRunning}
            >
              {laytimeRunning ? "Running..." : "Run laytime calculation"}{" "}
      ...
      <section
        className="mx-6 mt-3 rounded-lg border bg-white p-4"
        style={{ borderColor: "#E5E7EB" }}
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p style={{ fontSize: "11px", color: "#6B7280", marginTop: "3px" }}>
              Factual observations only. Candidate-linked evidence supports
              WIBON/WIPON evaluation; unassociated observations remain timeline
              evidence.
            </p>
          </div>
          <span style={{ fontSize: "11px", color: "#6B7280" }}>
            {locationEvidenceLoading
              ? "Loading..."
              : `${locationEvidence.length} observation(s)`}
          </span>
        </div>
        ...
        {locationQualification && (
          <div
            className="mt-3 grid gap-2 rounded-lg border p-3 sm:grid-cols-2 lg:grid-cols-4"
            style={{ borderColor: "#D1D5DB", backgroundColor: "#F9FAFB" }}
          >
            <CalcRow
              label="Berth qualification"
              value={locationQualification.berth?.status ?? "UNAVAILABLE"}
            />
            <CalcRow
              label="Port qualification"
              value={locationQualification.port?.status ?? "UNAVAILABLE"}
            />
            <CalcRow
              label="WIBON"
              value={
                wibonDecision?.applied
                  ? "Applied"
                  : wibonDecision?.configured
            ...
            <CalcRow
              label="WIPON"
              value={
                wiponDecision?.applied
                  ? "Applied"
                  : wiponDecision?.configured
                    ? "Configured / not applied"
                    : "Not configured"
              }
            />
            ...
            >
              {locationQualification.selectedEvidence
                ? `Evidence ${locationQualification.selectedEvidence.id} observed ${locationQualification.selectedEvidence.sourceTimeZone ? `${formatSourceDateTime(locationQualification.selectedEvidence.evidenceTime, locationQualification.selectedEvidence.sourceTimeZone)} (${locationQualification.selectedEvidence.sourceTimeZone})` : formatDateTime(locationQualification.selectedEvidence.evidenceTime)} from ${locationQualification.selectedEvidence.source}.`
          ...
          >
            <label style={{ fontSize: "11px", color: "#374151" }}>
              ...
              <input
                ...
                onChange={(event) =>
                  setLocationEvidenceForm((current) => ({
                    ...current,
                    evidenceTime: event.target.value,
                  }))
                }
              ...
              <span
                ...
                style={{ fontSize: "10px", color: "#6B7280" }}
              >
                Enter source-local civil time, not browser-local time.
              </span>
            </label>
            <label style={{ fontSize: "11px", color: "#374151" }}>
              Source timezone
              <input
                type="text"
                required
                value={locationEvidenceForm.sourceTimeZone}
                onChange={(event) =>
                  setLocationEvidenceForm((current) => ({
                    ...current,
                ...
                style={{ borderColor: "#D1D5DB" }}
              />
              {selectedNorTenderEvent?.sourceTimeZone && (
                <span
                  className="mt-1 block"
                  style={{ fontSize: "10px", color: "#6B7280" }}
                >
                  Inherited from the associated NOR tender.
                </span>
              )}
            </label>
            <label style={{ fontSize: "11px", color: "#374151" }}>
              ...
              <select
                ...
                onChange={(event) =>
                  setLocationEvidenceForm((current) => ({
                    ...current,
                    operation: event.target.value as "Loading" | "Discharge",
                  }))
                }
                className="mt-1 w-full rounded-md border bg-white px-2 py-2"
                style={{ borderColor: "#D1D5DB" }}
            ...
            <label style={{ fontSize: "11px", color: "#374151" }}>
              ...
              <select
                ...
                onChange={(event) =>
                  setLocationEvidenceForm((current) => ({
                    ...current,
                    source: event.target.value as "MANUAL" | "SOF",
                  }))
                }
                className="mt-1 w-full rounded-md border bg-white px-2 py-2"
                style={{ borderColor: "#D1D5DB" }}
              >
                ...
                <option value="SOF" disabled={!activeDocument}>
                  Current SOF document
                </option>
              </select>
            </label>
            <label style={{ fontSize: "11px", color: "#374151" }}>
              ...
              <select
                ...
                onChange={(event) =>
                  setLocationEvidenceForm((current) => ({
                    ...current,
                    portRelation: event.target.value as NorPortRelation,
                  }))
                }
                className="mt-1 w-full rounded-md border bg-white px-2 py-2"
                style={{ borderColor: "#D1D5DB" }}
            ...
            <label style={{ fontSize: "11px", color: "#374151" }}>
              ...
              <select
                ...
                onChange={(event) =>
                  setLocationEvidenceForm((current) => ({
                    ...current,
                    berthRelation: event.target.value as NorBerthRelation,
                  }))
                }
                className="mt-1 w-full rounded-md border bg-white px-2 py-2"
                style={{ borderColor: "#D1D5DB" }}
            ...
            <label style={{ fontSize: "11px", color: "#374151" }}>
              ...
              <select
                ...
                onChange={(event) =>
                  setLocationEvidenceForm((current) => ({
                    ...current,
                    waitingPlace: event.target.value as NorWaitingPlace,
                  }))
                }
                className="mt-1 w-full rounded-md border bg-white px-2 py-2"
                style={{ borderColor: "#D1D5DB" }}
              >
                ...
                <option value="CUSTOMARY_WAITING_PLACE">
            ...
            <label style={{ fontSize: "11px", color: "#374151" }}>
              ...
              <select
                ...
                onChange={(event) => {
                  const selected = norTenderEvents.find(
                    (item) => item.eventId === event.target.value,
                  );
                  setLocationEvidenceForm((current) => ({
                    ...current,
                    norTenderedEventId: event.target.value,
                    sourceTimeZone:
                      selected?.sourceTimeZone ?? current.sourceTimeZone,
                    evidenceTime: selected?.sourceTimeZone
                      ? formatDateTimeInputInTimeZone(
                          selected.eventTimeIso ?? "",
              ...
              >
                <option value="">
                  Unassociated observation (not used for v1 validity)
                </option>
                {norTenderEvents.map((event) => (
                  <option key={event.eventId} value={event.eventId}>
                    {event.sourceTimeZone
                      ? `${formatSourceDateTime(event.eventTimeIso ?? "", event.sourceTimeZone)} (${event.sourceTimeZone})`
              ...
              <span
                ...
                style={{ fontSize: "10px", color: "#6B7280" }}
              >
                Select the exact NOR tender this evidence describes. The
                evidence time must be at or before that tender.
              </span>
            </label>
            <label style={{ fontSize: "11px", color: "#374151" }}>
              Source reference
            ...
            <div className="flex items-end justify-end gap-2 md:col-span-3">
              <button
                type="button"
                onClick={() => setShowLocationEvidenceForm(false)}
              ...
              <button
                type="submit"
                disabled={savingLocationEvidence}
                className="rounded-md px-3 py-2 text-white"
                style={{
                  fontSize: "11px",
                  backgroundColor: "#1A4ED8",
                  opacity: savingLocationEvidence ? 0.7 : 1,
                }}
              >
        ...
        {!locationEvidenceLoading &&
          ...
          !locationEvidenceError && (
            <p className="mt-3" style={{ fontSize: "11px", color: "#6B7280" }}>
              No tender-location evidence has been recorded. No location is
              inferred.
            </p>
          )}
        {locationEvidence.length > 0 && (
          <div className="mt-3 grid gap-2 md:grid-cols-2">
            {locationEvidence.map((evidence) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span style={{ fontSize: "10px", color: "#6B7280" }}>
                    {evidence.sourceTimeZone
                      ? `${formatSourceDateTime(evidence.evidenceTime, evidence.sourceTimeZone)} (${evidence.sourceTimeZone})`
                      : formatDateTime(evidence.evidenceTime)}
                  </span>
                </div>
                <p
                  className="mt-1"
                ...
                >
                  Port: {humanizeLabel(evidence.portRelation)} · Berth:{" "}
                  {humanizeLabel(evidence.berthRelation)}
                </p>
                <p style={{ fontSize: "11px", color: "#374151" }}>
                  Waiting place: {humanizeLabel(evidence.waitingPlace)} ·
      ...
      {claimSuccess && (
        ...
        >
          ...
          <p style={{ fontSize: "11px", lineHeight: 1.4 }}>
            {claimSuccess.replace("Claim created successfully. ", "")}
          </p>
        </div>
      )}
        ...
        {[
          {
            ...
            value: summaryAllowedDisplay,
            vc: "#1A4ED8",
            sub: hasLaytimeCalculation
              ? (singleOperationSummaryLabel ??
                "Backend result from charter party")
              : "No persisted calculation yet",
          },
          {
            label: "Laytime used",
            value: summaryUsedDisplay,
            vc: "#B45309",
            sub: hasLaytimeCalculation
              ? (singleOperationSummaryLabel ??
                "Gross elapsed time from the backend")
              : "No persisted calculation yet",
          },
          {
            label: "Deductions",
            ...
            vc: "#374151",
            sub: hasLaytimeCalculation
              ? "Backend exception periods"
              : "No persisted calculation yet",
          },
          {
            label: "Remaining",
            value: summaryRemainingDisplay,
            vc: "#22543D",
            sub: hasLaytimeCalculation
              ? summaryOverrunDisplay === "—"
                ? "Net laytime balance from backend"
                : summaryOverrunDisplay === "See operation results"
                  ? "See operation results"
                  : `${summaryOverrunDisplay} over`
              : "No persisted calculation yet",
          },
          {
            label: "Net position",
            ...
            sub: hasLaytimeCalculation
              ? reversibleConfigured
                ? reversibleSettlementStatusLabel(reversibleSettlementStatus)
                : monetarySummary?.status === "AVAILABLE"
                  ? "Informational only - operation results remain separate"
                  : "Authoritative calculation currency unavailable"
              : "No persisted calculation yet",
          },
      ...
      <div className="flex gap-3.5 flex-1" style={{ padding: "16px 24px" }}>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <span
                ...
                style={{
                  fontSize: "10px",
                  backgroundColor: sourceStatusBg,
                  color: sourceStatusText,
                }}
              >
                {sourceStatusLabel}
              </span>
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-1.5">
                {[
                  {
                    label: `${eventCounts.normal} recorded`,
                    bg: "#C6F6D5",
                    text: "#22543D",
                  },
                  {
                    label: `${eventCounts.deductible} review`,
                    bg: "#F3F4F6",
                    text: "#374151",
                  },
                  {
                    label: `${eventCounts.pending} pending`,
                    bg: "#EFF6FF",
                    text: "#1E40AF",
                  },
                ].map(({ label, bg, text }) => (
            ...
            {eventCounts.deductible + eventCounts.pending > 0 && (
              ...
              >
                ...
                >
                  {eventCounts.deductible + eventCounts.pending} event(s) are
                  marked for review in the persisted SOF evidence.
                </p>
              </div>
            )}
              ...
              >
                <thead>
                  <tr style={{ borderBottom: "0.5px solid #E5E7EB" }}>
                    <th
                      className="pl-4 py-2.5 text-left"
                      style={{ width: "36px" }}
                    >
                      <span
                        style={{
                  ...
                  {displayEvents.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-4 py-10 text-center">
                        ...
                        >
                          No Statement of Facts events yet.
                        </p>
                        <p
                          style={{
                            fontSize: "11px",
                        ...
                        >
                          Add the first operational event to begin building the
                          laytime evidence timeline.
                        </p>
                      </td>
                    </tr>
                  ) : (
                        ...
                        >
                          <span
                            style={{
                              fontSize: "12px",
                              color: "#374151",
                              fontWeight: ev.duration.includes("h") ? 500 : 400,
                            }}
                          >
                            {ev.duration}
                          </span>
                        ...
                        >
                          <span
                            ...
                            style={{
                              ...
                              backgroundColor: ev.causeActive
                                ? "#EFF6FF"
                                : "#ffffff",
            ...
            {/* Add manual event */}
            <div
              style={{ borderTop: "0.5px solid #E5E7EB", padding: "10px 16px" }}
            >
              <button
                type="button"
        ...
        >
          ...
          >
            ...
            <CalcRow
              label="Deductions"
              value={formatSecondsAsInterval(deductionSeconds)}
              valueColor="#22543D"
            />
            <CalcRow
              label="Time balance"
            ...
            {reversibleConfigured && (
              <CalcRow
                ...
                value={reversibleSettlementStatusLabel(
                  reversibleSettlementStatus,
                )}
              ...
              {operationResultsError ? (
                ...
                <div className="space-y-3">
                  {operationResults.map((result) => {
                    const operationResultSnapshot =
                      getOperationResultSnapshot(result);
                    const operationLabel = formatOperationLabel(
                      result.operation,
                    );
                    const sourceLine =
                    ...
                    return (
                      ...
                      >
                        ...
                        <CalcRow
                          label="Allowed laytime"
                          value={formatInterval(result.allowedLaytime)}
                        />
                        <CalcRow
                          label="Used laytime"
                          value={formatInterval(result.usedLaytime)}
                        />
                        <CalcRow
                          label={
                            reversibleSettlementStatus === "FINAL_AUTHORITATIVE"
                              ? "Demurrage (reference only)"
                              : "Demurrage"
                          }
                          value={formatCurrencyAmount(
                            result.demurrageAmount,
                            result.currency,
                          )}
                        />
                        <CalcRow
                          label={
                            reversibleSettlementStatus === "FINAL_AUTHORITATIVE"
                              ? "Despatch (reference only)"
                              : "Despatch"
                          }
                          value={formatCurrencyAmount(
                            result.despatchAmount,
                            result.currency,
                          )}
                        />
                ...
                >
                  No operation-specific child results available for this
                  calculation.
                </p>
              )}
            </div>
          ...
          {demurrageAuditVisible && (
            ...
            >
              ...
              >
                Once demurrage started, these later exceptions did not suspend
                the clock.
              </p>
              <CalcRow
              ...
              {!reversibleLaytimeAudit.hasAnalysis ? (
                ...
                <>
                  ...
                  <CalcRow
                    ...
                    value={
                      reversibleLaytimeAudit.contractRuleApplied ? "Yes" : "No"
                    }
                    valueColor="#374151"
                  />
                  <p
                ...
                <>
                  <CalcRow
                    ...
                    valueColor={
                      reversibleLaytimeAudit.contractRuleApplied
                        ? "#22543D"
                        : "#6B7280"
                    }
                    bold
                  ...
                  <CalcRow
                    ...
                    value={formatSecondsAsInterval(
                      reversibleLaytimeAudit.loading?.allowedSeconds,
                    )}
                    valueColor="#1E40AF"
                  />
                  <CalcRow
                    label="Loading used"
                    value={formatSecondsAsInterval(
                      reversibleLaytimeAudit.loading?.usedSeconds,
                    )}
                    valueColor="#374151"
                  />
                  <CalcRow
                    ...
                    value={formatSecondsAsInterval(
                      reversibleLaytimeAudit.loading?.surplusSeconds,
                    )}
                    valueColor="#22543D"
                  />
                  <CalcRow
                    ...
                    value={formatSecondsAsInterval(
                      reversibleLaytimeAudit.loading?.overrunSeconds,
                    )}
                    valueColor="#B45309"
                  />
                  <CalcRow
                    ...
                    value={formatSecondsAsInterval(
                      reversibleLaytimeAudit.discharge?.allowedSeconds,
                    )}
                    valueColor="#1E40AF"
                  />
                  <CalcRow
                    ...
                    value={formatSecondsAsInterval(
                      reversibleLaytimeAudit.discharge?.usedSeconds,
                    )}
                    valueColor="#374151"
                  />
                  <CalcRow
                    ...
                    value={formatSecondsAsInterval(
                      reversibleLaytimeAudit.discharge?.surplusSeconds,
                    )}
                    valueColor="#22543D"
                  />
                  <CalcRow
                    ...
                    value={formatSecondsAsInterval(
                      reversibleLaytimeAudit.discharge?.overrunSeconds,
                    )}
                    valueColor="#B45309"
                  />
                  <div
                  ...
                  >
                    <CalcRow
                      label="Total allowed"
                      value={formatSecondsAsInterval(
                        reversibleLaytimeAudit.pool?.totalAllowedSeconds,
                      )}
                      valueColor="#1E40AF"
                    />
                    <CalcRow
                      label="Total used"
                      value={formatSecondsAsInterval(
                        reversibleLaytimeAudit.pool?.totalUsedSeconds,
                      )}
                      valueColor="#374151"
                    />
                    <CalcRow
                      ...
                      value={formatSecondsAsInterval(
                        reversibleLaytimeAudit.pool?.transferableSurplusSeconds,
                      )}
                      valueColor="#22543D"
                    />
                    <CalcRow
                      ...
                      value={formatSecondsAsInterval(
                        reversibleLaytimeAudit.pool?.netPooledOverrunSeconds,
                      )}
                      valueColor="#B45309"
                    />
                    <CalcRow
                      ...
                      value={formatSecondsAsInterval(
                        reversibleLaytimeAudit.pool?.netPooledSurplusSeconds,
                      )}
                      valueColor="#22543D"
                    />
                  </div>
              ...
              {!sofDocumentSelectionAudit.available ? (
                ...
                <>
                  <CalcRow
                    ...
                    value={
                      sofDocumentSelectionAudit.voyageLaytimeOperation ??
                      "Not set"
                    }
                    valueColor="#1E40AF"
                    bold
                  ...
                  <CalcRow
                    ...
                    value={String(
                      sofDocumentSelectionAudit.candidateDocumentCount,
                    )}
                    valueColor="#374151"
                  />
                  <CalcRow
                    ...
                    value={String(
                      sofDocumentSelectionAudit.includedDocumentCount,
                    )}
                    valueColor="#22543D"
                    bold
                  />
                  <CalcRow
                    ...
                    value={String(
                      sofDocumentSelectionAudit.matchingDocumentCount,
                    )}
                    valueColor="#374151"
                  />
                  <CalcRow
                    ...
                    value={String(
                      sofDocumentSelectionAudit.legacyNullDocumentCount,
                    )}
                    valueColor="#374151"
                  />
                  <CalcRow
                    ...
                    value={String(
                      sofDocumentSelectionAudit.oppositeOperationDocumentCount,
                    )}
                    valueColor="#9A3412"
                  />
                  {sofDocumentSelectionAudit.hasLegacyNullDocuments && (
                    ...
                    >
                      Legacy SOF documents without operation context were
                  ...
                  {sofDocumentSelectionAudit.hasOppositeOperationDocuments && (
                    ...
                    >
                      SOF documents explicitly assigned to the opposite laytime
              ...
              {!operationSelectionAudit.available ? (
                ...
                <>
                  <CalcRow
                    label="Voyage laytime operation"
                    value={
                      operationSelectionAudit.voyageLaytimeOperation ??
                      "Not set"
                    }
                    valueColor="#1E40AF"
                    bold
                  ...
                  <CalcRow
                    ...
                    value={
                      operationSelectionAudit.hasLoadingCompletion === true
                        ? "Yes"
                        : "No"
                    }
                    valueColor="#374151"
                  ...
                  <CalcRow
                    ...
                    value={
                      operationSelectionAudit.hasDischargeCompletion === true
                        ? "Yes"
                        : "No"
                    }
                    valueColor="#374151"
                  ...
                  {operationSelectionAudit.mixedOperationEvidence === true && (
                    ...
                    >
                      Both Loading and Discharge completion evidence were
                      present. The calculation used the voyage laytime operation
                      to select the applicable completion event.
                    </p>
                  )}
                  {(operationSelectionAudit.excludedCompletionCount ?? 0) >
                    0 && (
                    <CalcRow
                      label="Excluded mismatched completion events"
                      value={String(
                        operationSelectionAudit.excludedCompletionCount,
                      )}
                      valueColor="#9A3412"
                      bold
                    />
